...
(ingot :id "v_auto_c1" :status forged :solo t :grade 2 :skill cli :heat 1 :max 5 :smelt 1 :proof "cargo clippy -- -D warnings && cargo test --all" :work "Verify zero clippy warnings and all tests pass after needless borrow fix in src/config.rs. Use exit codes directly — RTK proxy reformats output so grep patterns fail.")
